...
@IBDesignable open class PillPageControl: UIView {
    ...
    // MARK: - Appearance
    @IBInspectable open var pillSize: CGSize = CGSize(width: 12, height: 3) {
        didSet {
    ...
    @IBInspectable open var inactiveTint: UIColor = UIColor(white: 1, alpha: 0.5) {
        didSet {
    ...
    fileprivate lazy var activeLayer: CALayer = { [unowned self] in
        let layer = CALayer()
        layer.frame = CGRect(origin: CGPoint(x: 0, y: -0.75),
                             size: CGSize(width: self.pillSize.width, height: self.pillSize.height + 1.5))
        layer.backgroundColor = self.activeTint.cgColor
        layer.cornerRadius = (self.pillSize.height + 1.5)/2
        layer.actions = [
            "bounds": NSNull(),
            "frame": NSNull(),
            "position": NSNull()]
